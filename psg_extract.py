...
Brug:
  # Enkelt fil
  python "psg_extract copy.py" rapport.txt --token DIT_HF_TOKEN

  # Batch (alle .txt i en mappe)
  python "psg_extract copy.py" --mappe forberedt/ --output-mappe resultater/ --token DIT_HF_TOKEN
"""
...
def load_local_model(token):
    ...
    print("Indlæser lokal model...")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, token=token)
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME,
        token=token,
        dtype=torch.float32,
        device_map="auto" # Udnytter GPU hvis tilgængelig
    )
    model.eval()
...
def extract_from_text(tokenizer, model, text: str, max_retries: int = 3) -> dict:
    ...
    raise RuntimeError(f"Udtrækning fejlede efter {max_retries} forsøg: {last_error}")
...
def process_batch(
    ...
    for txt_file in tqdm(txt_files, unit="fil"):
        ...
        try:
            ...
            process_file(txt_file, out_file, tokenizer, model, verbose=verbose)
            ok += 1
...
def main():
    ...
    tokenizer, model = load_local_model(token)

    if args.input:
        input_path  = Path(args.input)
        output_path = Path(args.output) if args.output else input_path.with_suffix(".json")
        result = process_file(input_path, output_path, tokenizer, model, verbose=True)
    ...
    if args.mappe:
        ...
        process_batch(input_dir, output_dir, tokenizer, model, verbose=args.verbose)
        return
